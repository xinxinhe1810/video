...
def modify_host_file():
    ...
    i = 0
    ips = []
    print('modify_host_file')
    while i < 5:
        i += 1
        ips = get_tmdb_ip_ip33()
        if ips:
            print('get_tmdb_ip_ip33', ips)
            break
        else:
            ips = get_tmdb_ip_myssl()
            if ips:
                print('get_tmdb_ip_myssl', ips)
                break
            else:
                time.sleep(30)
...
if __name__ == '__main__':
    ...
    if flag == 1:
        run_media()
        schedule.every(interval).minute.do(run_media)  # 0706
        while True:
